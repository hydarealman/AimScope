...
bool http_conn::add_content_type()
{
    ...
    // 根据实际请求的文件名（m_real_file）判断扩展名
    const char *filename = m_real_file;
    const char *ext = strrchr(filename, '.');
    if (!ext) {
        // 没有后缀，统一按文本处理
        return add_response("Content-Type: text/plain; charset=utf-8\r\n");
    }
    
    if (strcasecmp(ext, ".html") == 0 || strcasecmp(ext, ".htm") == 0) {
        return add_response("Content-Type: text/html; charset=utf-8\r\n");
    } else if (strcasecmp(ext, ".js") == 0) {
        return add_response("Content-Type: application/javascript; charset=utf-8\r\n");
    } else if (strcasecmp(ext, ".css") == 0) {
        return add_response("Content-Type: text/css; charset=utf-8\r\n");
    } else if (strcasecmp(ext, ".jpg") == 0 || strcasecmp(ext, ".jpeg") == 0) {
        return add_response("Content-Type: image/jpeg\r\n");
    } else if (strcasecmp(ext, ".png") == 0) {
        return add_response("Content-Type: image/png\r\n");
    } else if (strcasecmp(ext, ".gif") == 0) {
        return add_response("Content-Type: image/gif\r\n");
    } else if (strcasecmp(ext, ".mp4") == 0) {
        return add_response("Content-Type: video/mp4\r\n");
    } else if (strcasecmp(ext, ".ico") == 0) {
        return add_response("Content-Type: image/x-icon\r\n");
    } else {
        // 默认按文本处理，带utf-8
        return add_response("Content-Type: text/plain; charset=utf-8\r\n");
    }
